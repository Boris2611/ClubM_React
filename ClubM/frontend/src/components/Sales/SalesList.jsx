...
import { createSaleItem, updateSaleItem, deleteSaleItem } from '../../utils/api';

const SalesList = ({ sales = [], loading, refresh, onOrder }) => {
  const { user } = useAuth();
  ...
  const handleEditSave = async (e) => {
    ...
    try {
      ...
      await updateSaleItem(editModal.item._id, formData);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-3xl font-bold text-gray-800">Kupi:</h2>
        {user?.role === 'admin' && (
          <button
          ...
          : sales.map(sale => (
              <div key={sale._id} className="bg-white rounded-xl shadow-lg overflow-hidden flex flex-col">
                ...
                <div className="p-4 flex-1 flex flex-col">
                  ...
                  <div className="mt-2 text-lg font-bold text-green-600">{sale.price} RSD</div>
                  {user?.role === 'admin' ? (
                    ...
                    <button
                      onClick={() => onOrder && onOrder(sale)}
                      className="bg-blue-600 text-white rounded px-4 py-1 font-semibold hover:bg-blue-700 transition mt-2 self-end"
                    >
                      Naruči
                    </button>
                  ) : null}
